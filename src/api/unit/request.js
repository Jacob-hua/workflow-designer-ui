...
service.interceptors.request.use(
  (config) => {
    ...
    const meosToken = (sessionStorage.getItem('meosToken') && sessionStorage.getItem('meosToken')) || ''
    if (meosToken) {
      config.headers['Siact-Token'] = meosToken
    ...
    // config.headers['Workflow-Platform-Authorization'] = '51cf84f3-948a-4e3e-9872-9f69c1d1977e'
    return config
  },
  (error) => {
