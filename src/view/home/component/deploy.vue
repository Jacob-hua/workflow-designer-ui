  ...
  <div>
    <el-dialog title="部署工作流"
      ...
      <div>
        <div class="from-item">
          <span>应用项目</span>
          <el-input v-model="firstData.ascription"
                    placeholder="请输入应用项目"
        ...
        <div class="from-item">
          <span>能源系统</span>
          <el-select v-model="firstData.systemType"
                     placeholder="请选择能源系统">
        ...
        <div class="from-item">
          <span>部署名称</span>
          <el-input v-model="firstData.deployName"
                    placeholder="请输入部署名称"></el-input>
        </div>
      </div>
      <span slot="footer"
            class="dialog-footer">
        <el-button @click="nextDiolog()"
    ...
    <el-dialog title="部署工作流"
      ...
      <div class="dialogVisible2-left">
        <div class="bpmn-Main">
          <ProcessInformation type="deploy2"
                              ref="ProcessInformation"
                              v-if="dialogVisible2"
                              @selection="selection"></ProcessInformation>
        </div>
        <div class="bpmn-configure">
          ...
          <div class="bpmn-configure-form">
            ...
            <div class="bpmn-configure-Main">
              ...
              <span v-if="formShow"
                    class="formRemove"
                    @click="removeForm()">移除表单</span>
              <div v-if="formShow"
                   class="formShowForm">
                <formOB v-if="formShow && (formContent.docType === '.form' || formContent.docType === null)"
                        :formContant="formContent.content"
                        :key="formOBKey"></formOB>
                <preview :itemList="formListFun(formContent)"
                         :formConf="configFun(formContent)"
                         v-if="formShow && formContent.docType === 'json'"></preview>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="dialogVisible2-right">
        ...
        <div class="dialogVisible2-right-search">
          <el-input v-model="input"
                    placeholder="请输入内容"
                    @keyup.native.enter="getFormList()"></el-input>
        </div>
        <div class="formList">
          <div class="listItem"
            ...
            <div class="listItem-button">
              <el-popover placement="right"
                          width="400"
                          trigger="click">
                <formOB :formContant="item.content"
                        v-if="item.docType === '.form' || item.docType === null"></formOB>
                <preview :itemList="formListFun(item)"
                         :formConf="configFun(item)"
                         v-if="item.docType === 'json'"></preview>
                <el-button type="text"
                           size="small"
                           class="listItem-button1"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="addWorkFlow()"
                   v-role="{ id: 'HomeDeploy', type: 'button', business: business }">部署</el-button>
  ...
  methods: {
    ...
    addWorkFlow() {
      ...
      this.bpmnModeler.saveXML({ format: true }).then(({ xml }) => {
        let a = xml.replaceAll(
          this.$refs.ProcessInformation.postData.code,
          ...
          if (Array.isArray(definitions.process.userTask)) {
            definitions.process.userTask.forEach((item, index) => {
              if (item['_camunda:formKey']) {
                formIds =
                  formIds +
                  item['_camunda:formKey'].split('.')[1].split('_')[1] +
                  ','
              }
            })
          } else {
        ...
        switch (this.dataType) {
          case 'enabled':
            formData.append(
              'processId',
              this.$refs.ProcessInformation.postData.id
            )
            break
          case 'drafted':
            formData.append(
              'processId',
              this.$refs.ProcessInformation.postData.processId
            )
        ...
        formData.append(
          'deployName',
          this.$refs.ProcessInformation.postData.deployName
        )
    ...
    nextDiolog() {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.ProcessInformation.postData.ascription =
          this.firstData.ascription
        this.$refs.ProcessInformation.postData.business =
          this.firstData.business
        this.$refs.ProcessInformation.postData.systemType =
          this.firstData.systemType
        this.$refs.ProcessInformation.postData.deployName =
          this.firstData.deployName
        this.getFormList()
