  ...
  <div>
      ...
      <div v-if="dialogVisible1" class="details-footer-height">
        <div class="details-header">
          ...
          <span class="frequency"
            >已部署次数:
            {{ DeployBasicList[postData.systemType] ? DeployBasicList[postData.systemType].length : '0' }}</span
          >
        </div>
        <div class="detail-list">
          <div
            class="detail-list-item"
            v-if="DeployBasicList[postData.systemType] && DeployBasicList[postData.systemType].length > 0"
            v-for="(item, index) in DeployBasicList[postData.systemType]"
            :key="index"
          >
          ...
          <div style="line-height: 194px; width: 100%; text-align: center" v-if="!DeployBasicList[postData.systemType]">
            暂无数据
          </div>
        </div>
      </div>
    ...
    <el-dialog title="部署工作流" :visible.sync="dialogVisible2" width="70%" custom-class="dialogVisible1">
      ...
      <span slot="footer" class="dialog-footer" v-if="seeType === 'delete'">
        <el-button @click="deleteDeployment()" v-role="{ id: 'HomeDelete', type: 'button', business: business }"
          >删除</el-button
        >
      </span>
    </el-dialog>
