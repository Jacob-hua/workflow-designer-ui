  ...
  <div class="ProcessInformation">
    <div class="ProcessInformation-title">
      ...
      <div class="title-item">
        ...
        <div class="title-item-main">
          <el-input v-model="postData.deployName"
                    placeholder=""
                    :disabled="true"></el-input>
        </div>
      </div>
    ...
    <div class="ProcessInformation-bpmn">
      <span class="bpmn-Main-title"
            v-if="type == 'deploy2'">BPMN流程</span>
      ...
      try {
        ...
        this.$nextTick(() => {
          if (id) {
            ...
            let userTaskComplateArr =
              window.bpmnInstances.elementRegistry.filter((element) => {
                return userTask.indexOf(element.id) !== -1
              })

            let userTaskArr = window.bpmnInstances.elementRegistry.filter(
              (element) => {
                return element.type === 'bpmn:UserTask'
              }
            )
            let oneSet = window.bpmnInstances.elementRegistry.filter(
              (element) => {
