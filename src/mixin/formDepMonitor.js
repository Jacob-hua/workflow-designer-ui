...
export function variableClassify(
  { variable, sourceType, source },
  variableSpace = {}
) {
...
export function watchExecute(
  ...
  function calculateDependValue(data, fieldValuePath, dependValuePath) {
    ...
    const domainPath = fieldValuePath.substring(
      0,
      fieldValuePath.lastIndexOf(".")
    );
    const domain =
      domainPath === "" ? data : _.get(data, domainPath, undefined);
    if (!domain) {
      return;
    }
  ...
  function diffeDepObj(dependObj, data) {
    return Object.keys(dependObj).reduce((isDiffe, dependValuePath) => {
      const dependValue = calculateDependValue(
        data,
        fieldInfo.valuePath,
        dependValuePath
      );
...
export function mixinRequestFunction(fieldInfo, executeFunc = () => {}) {
  ...
  const variableSpace = variables.reduce(
    (variableSpace, metaVariable) =>
      variableClassify(metaVariable, variableSpace),
    {
...
function formDepMonitorMixin(
  props = { formData: "formData", formFields: "formFields" }
) {
    ...
    watch: {
      [formData]: {
        ...
        handler(data) {
          this.$nextTick(() => {
            const needExecutes =
              this[formFields]?.filter(({ executeFuncs }) => executeFuncs) ??
              [];
